...
import { Building, Users, DollarSign, TrendingUp, Clock, ArrowRight } from 'lucide-react'
...
export default async function AdminOverview() {
    ...
    // Fetch pending shops and their emails
    const { data: pendingShopsData } = await supabase
        .from('shops')
    ...
    const pendingData = pendingShopsData?.map((shop: any) => {
        const user = users.find(u => u.id === shop.owner_id)
        return { ...shop, email: user?.email || 'Unknown' }
    }) || []
        ...
        <div className="space-y-8 p-4 lg:p-8">
            ...
            <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm">
                <h3 className="flex items-center gap-2 text-lg font-bold text-gray-900 dark:text-white mb-4">
                    <Clock size={20} className="text-orange-500" /> Pending Setup
                </h3>
                <div className="space-y-3">
                    {pendingData.length > 0 ? (
                        pendingData.map((shop: any) => (
                            <div key={shop.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-gray-100 dark:border-gray-800 pb-2">
                                <div>
                                ...
                                >
                                    Go to Shops to Share Link <ArrowRight size={12} />
                                </Link>
                            </div>
                        ))
                    ) : (
