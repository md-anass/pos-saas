...
type DealComponent = { product_id: string; quantity: number }
type Deal = { id: string; name: string; deal_price: number; summary: string; components: DealComponent[] }
...
export default function RestaurantPOSWorkspace({ order, items: initialItems, products, categories, deals, currency }: { order: Order; items: OrderItem[]; products: Product[]; categories: Category[]; deals: Deal[]; currency: string }) {
    ...
    const dealQueues = useRef(new Map<string, Promise<void>>())
    const [dealPending, setDealPending] = useState<Set<string>>(new Set())
    ...
    // Refresh local cart rows after a server action revalidates the order.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    useEffect(() => setItems(initialItems), [initialItems])
    const dealLines = deals.map(deal => {
        const rows = items.filter(item => item.notes?.split(' | ').some(note => note.startsWith(`Deal: ${deal.name}`)))
        const quantity = deal.components.length ? Math.floor(Math.min(...deal.components.map(component => {
            const row = rows.find(item => item.product_id === component.product_id)
            return row ? Number(row.quantity) / Number(component.quantity) : 0
        }))) : 0
        return { deal, quantity, rows }
    }).filter(line => line.quantity > 0)
    const dealRowIds = new Set(dealLines.flatMap(line => line.rows.map(item => item.id)))
    const regularItems = items.filter(item => !dealRowIds.has(item.id) && !item.notes?.startsWith('Deal:'))
    const total = regularItems.reduce((sum, item) => sum + Number(item.quantity) * Number(item.unit_price), 0) + dealLines.reduce((sum, line) => sum + line.quantity * Number(line.deal.deal_price), 0)
    ...
    const mutateDeal = (deal: Deal, delta: number) => {
        const key = `${order.id}:${deal.id}`
        if (dealQueues.current.has(key)) return
        const run = (async () => {
            setDealPending(current => new Set(current).add(key))
            try {
                if (delta > 0) {
                    const data = new FormData()
                    data.set('order_id', order.id)
                    data.set('deal_id', deal.id)
                    data.set('quantity', String(delta))
                    await addDealToOrder(data)
                } else {
                    for (const component of deal.components) {
                        const data = new FormData()
                        data.set('order_id', order.id)
                        data.set('product_id', component.product_id)
                        data.set('delta', String(-component.quantity * Math.abs(delta)))
                ...
                setError('Could not update this deal. Please try again.')
            }
        })()
        dealQueues.current.set(key, run)
        void run.finally(() => {
            if (dealQueues.current.get(key) === run) dealQueues.current.delete(key)
            setDealPending(current => {
    ...
    const flushPending = async () => {
        while (queues.current.size > 0) {
            const pending = Array.from(queues.current.entries())
            const settled = await Promise.allSettled(pending.map(([, promise]) => promise))
            settled.forEach((result, index) => {
                const [key, promise] = pending[index]
                if (queues.current.get(key) === promise) queues.current.delete(key)
            })
    ...
    return <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-4 rounded-2xl border bg-white p-4 dark:bg-gray-900">
            ...
            {!!deals.length && <div className="border-t pt-4"><div className="mb-2"><h3 className="font-bold">Deals and combos</h3><p className="text-xs text-gray-500">A deal is charged once at its deal price. Its component items are not separate charges.</p></div><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{deals.map(deal => <form key={deal.id} onSubmit={event => { event.preventDefault(); mutateDeal(deal, 1) }} className="rounded-lg border border-orange-200 bg-orange-50/50 p-3 dark:bg-orange-950/20"><input type="hidden" name="order_id" value={order.id} /><input type="hidden" name="deal_id" value={deal.id} /><input type="hidden" name="quantity" value="1" /><div className="flex justify-between gap-2"><div className="min-w-0"><b className="block truncate"><span className="mr-1 rounded bg-orange-200 px-1 text-[10px] text-orange-800">DEAL</span>{deal.name}</b><p className="text-xs text-gray-500">{deal.summary}</p></div><b className="shrink-0">{money(deal.deal_price)}</b></div><button disabled={dealPending.has(`${order.id}:${deal.id}`)} className="mt-2 w-full rounded-lg bg-orange-600 p-2 text-sm text-white disabled:opacity-60">{dealPending.has(`${order.id}:${deal.id}`) ? 'Adding...' : 'Add deal'}</button></form>)}</div></div>}
        </div>
        <aside className="h-fit rounded-2xl border bg-white p-4 shadow-sm dark:bg-gray-900 lg:sticky lg:top-24">
            ...
            {!regularItems.length && !dealLines.length && <p className="py-8 text-center text-sm text-gray-500">Add menu items or a deal to begin.</p>}
            <div className="my-3 space-y-2">{regularItems.map(item => { const name = Array.isArray(item.products) ? item.products[0]?.name : item.products?.name; return <div key={item.id} className="rounded-lg border p-2"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-sm font-semibold">{name}</p><p className="text-xs text-gray-500">{money(Number(item.unit_price))} each</p></div><b className="shrink-0 text-sm">{money(Number(item.quantity) * Number(item.unit_price))}</b></div>{open && <div className="mt-2 flex items-center justify-end gap-2"><button type="button" aria-label={`Decrease ${name}`} onClick={() => queueDelta(item.product_id, -1)} className="h-7 w-7 rounded border">-</button><span className="min-w-5 text-center text-sm font-bold">{item.quantity}</span><button type="button" aria-label={`Increase ${name}`} onClick={() => queueDelta(item.product_id, 1)} className="h-7 w-7 rounded border">+</button></div>}</div> })}{dealLines.map(({ deal, quantity }) => <div key={deal.id} className="rounded-lg border border-orange-200 bg-orange-50/40 p-2"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-sm font-semibold"><span className="mr-1 rounded bg-orange-100 px-1 text-[10px] text-orange-700">DEAL</span>{deal.name}</p><p className="text-xs text-gray-500">{money(deal.deal_price)} each</p></div><b className="shrink-0 text-sm">{money(quantity * Number(deal.deal_price))}</b></div>{open && <div className="mt-2 flex items-center justify-end gap-2"><button type="button" disabled={dealPending.has(`${order.id}:${deal.id}`)} aria-label={`Decrease one ${deal.name}`} onClick={() => mutateDeal(deal, -1)} className="h-7 w-7 rounded border disabled:opacity-60">-</button><span className="min-w-5 text-center text-sm font-bold">{quantity}</span><button type="button" disabled={dealPending.has(`${order.id}:${deal.id}`)} aria-label={`Increase one ${deal.name}`} onClick={() => mutateDeal(deal, 1)} className="h-7 w-7 rounded border disabled:opacity-60">+</button><button type="button" disabled={dealPending.has(`${order.id}:${deal.id}`)} aria-label={`Remove ${deal.name}`} onClick={() => mutateDeal(deal, -quantity)} className="ml-2 rounded border px-2 py-1 text-xs text-red-600 disabled:opacity-60">Remove</button></div>}</div>)}</div>
            <div className="border-t pt-3"><div className="flex justify-between text-lg font-black"><span>Total</span><span>{money(total)}</span></div>{open && (regularItems.length > 0 || dealLines.length > 0) && <form action={payRestaurantOrder} onSubmit={handleCheckout} className="mt-3 grid gap-2"><input type="hidden" name="id" value={order.id} /><input type="hidden" name="change" value={Math.max(0, change).toFixed(2)} />            {paymentMethod === 'cash' && <div className="grid gap-1"><label htmlFor="received-cash" className="text-xs font-semibold text-gray-600 dark:text-gray-300">Received Cash</label><input id="received-cash" name="received_cash" value={receivedCash} onChange={event => setReceivedCash(event.target.value)} inputMode="decimal" placeholder="" className="rounded-lg border p-2.5" />{receivedCash && change >= 0 && <p className="text-right text-sm font-semibold text-emerald-600">Change: {money(change)}</p>}{receivedCash && change < 0 && <p className="text-xs text-red-600">Received cash must be at least the total amount.</p>}</div>}<select name="payment_method" value={paymentMethod} onChange={event => setPaymentMethod(event.target.value)} className="rounded-lg border p-2.5"><option value="cash">Cash</option><option value="card">Card</option><option value="bank_transfer">Bank transfer</option></select><button disabled={checkoutBusy} className="rounded-lg bg-emerald-600 px-4 py-3 font-bold text-white disabled:opacity-60">{checkoutBusy ? 'Processing...' : `Pay ${money(total)}`}</button></form>}{order.sale_id && <Link href={`/dashboard/sales/${order.sale_id}/receipt`} className="mt-3 block rounded-lg bg-emerald-600 px-4 py-3 text-center font-bold text-white">View / print receipt</Link>}</div>
        </aside>
    </section>
}
