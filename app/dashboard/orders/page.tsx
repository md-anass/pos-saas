...
export default async function OrdersPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
    ...
    if (selected) {
        const [{ data: order }, { data: items }, { data: products }, { data: categories }, { data: deals }] = await Promise.all([
            ...
            supabase.from('restaurant_deals').select('id,name,deal_price,restaurant_deal_items(product_id,quantity,products(name))').eq('shop_id', context.shop.id).eq('is_active', true).order('name'),
        ])
        if (order) workspace = <RestaurantPOSWorkspace key={selected} order={order} items={items || []} products={products || []} categories={categories || []} deals={(deals || []).map(deal => ({ id: deal.id, name: deal.name, deal_price: deal.deal_price, summary: (deal.restaurant_deal_items || []).map(item => { const product = Array.isArray(item.products) ? item.products[0] : item.products; return `${item.quantity} x ${product?.name || 'Item'}` }).join(', '), components: (deal.restaurant_deal_items || []).map(item => ({ product_id: item.product_id, quantity: Number(item.quantity) })) }))} currency={context.shop.currency} />
    }
